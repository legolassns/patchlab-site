...
- **Ricezione su info@patchlab.net**: dopo il test in sezione G, l'email deve arrivare con oggetto "Nuova richiesta preventivo PatchLab [LINGUA] — [Nome o Azienda]" (dal 2026-08-31 l'oggetto include l'etichetta della lingua: `EN`, `IT`, in futuro `FR`, oppure "non indicata") e tutti i campi leggibili (data/ora, **lingua richiesta**, nome, azienda, email, telefono, tipologia, applicazione, quantità, note). La riga di provenienza in fondo deriva anch'essa dalla lingua: prima era la stringa fissa "(it/preventivo/)", **falsa su ogni richiesta inglese**.
...
- **Campo `lingua`** (dal 2026-08-31): campo nascosto, non compilato dall'utente, che dichiara la lingua della pagina di origine (`en` sul form inglese, `it` su quello italiano). Validato contro la whitelist chiusa `LINGUE_AMMESSE` (`en`/`it`/`fr`) da `read_lingua_field()`. A differenza degli altri campi **non fa mai fallire l'invio**: un valore assente, vuoto o non riconosciuto diventa stringa vuota e viene reso come "non indicata". Il fallback non indovina una lingua di default — una richiesta da una pagina in cache, precedente all'introduzione del campo, deve risultare ignota, non essere attribuita all'italiano. `fr` è già ammesso in previsione della versione francese, che **non è ancora pubblicata**.
