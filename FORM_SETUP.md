...
**Modificati (2026-07-23, MODE 6C — CRO)**:
- `quote/index.html`, `it/preventivo/index.html` — aggiunto il campo opzionale "Intended date of use"/"Data di utilizzo indicativa" (`data-utilizzo`), per allineare il form alla promessa già presente nel copy del sito (checklist di `how-we-work/`/`come-lavoriamo/`, CTA finale della home) che la citava come informazione utile senza che il form la raccogliesse davvero. Testo libero, non un date-picker: coerente con "quantità indicativa" (nessuna precisione forzata).
- `api/invia-preventivo.php` — accetta il nuovo campo opzionale (`MAX_LEN_DATA_UTILIZZO = 100`, stesso pattern di validazione degli altri campi opzionali), incluso nel corpo dell'email (testo e HTML). Nessuna modifica alla logica di sicurezza/anti-spam.
...
- **Ricezione su info@patchlab.net**: dopo il test in sezione G, l'email deve arrivare con oggetto "Nuova richiesta preventivo PatchLab [LINGUA] — [Nome o Azienda]" (dal 2026-08-31 l'oggetto include l'etichetta della lingua: `EN`, `IT`, `FR`, in futuro `ES`, oppure "non indicata") e tutti i campi leggibili (data/ora, **lingua richiesta**, nome, azienda, email, telefono, tipologia, applicazione, quantità, note). La riga di provenienza in fondo deriva anch'essa dalla lingua: prima era la stringa fissa "(it/preventivo/)", **falsa su ogni richiesta inglese**.
...
- **Validazione campi**: whitelist esplicita di 9 campi compilati dall'utente (nome, azienda, email, telefono, tipo-patch, applicazione, quantita, `data-utilizzo`, note — quest'ultimo aggiunto nel MODE 6C per allinearsi alla promessa già presente nel copy del sito, vedi sezione "Evoluzioni future" più sotto), ciascuno con limite di lunghezza, tipo atteso ed enumerazione dove pertinente (tipo-patch, applicazione). Nessun campo arbitrario viene mai accettato o inoltrato.
- **Campo `lingua`** (dal 2026-08-31): campo nascosto, non compilato dall'utente, che dichiara la lingua della pagina di origine (`en` sul form inglese, `it` su quello italiano). Validato contro la whitelist chiusa `LINGUE_AMMESSE` (`en`/`it`/`fr`/`es`) da `read_lingua_field()`. A differenza degli altri campi **non fa mai fallire l'invio**: un valore assente, vuoto o non riconosciuto diventa stringa vuota e viene reso come "non indicata". Il fallback non indovina una lingua di default — una richiesta da una pagina in cache, precedente all'introduzione del campo, deve risultare ignota, non essere attribuita all'italiano. `fr` è pubblicato dal 2026-08-31 (`value="fr"` su `/fr/devis/`). `es` è ammesso dal 2026-09-01 in previsione della versione spagnola, che **non è ancora pubblicata**.
